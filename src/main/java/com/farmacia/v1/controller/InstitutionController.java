...
import com.farmacia.v1.service.impl.InstitutionServiceImpl;
...
public class InstitutionController {
    ...
    public ResponseEntity<Page<InstitutionDTO>> getAllPaginated(
    ...
    @GetMapping("/{id}")
    public ResponseEntity<InstitutionDTO> getById(@PathVariable Integer id) {
        return ResponseEntity.ok(service.getById(id));
    }

    @PutMapping("/{id}")
    public ResponseEntity<InstitutionDTO> update(@PathVariable Integer id, @RequestBody InstitutionDTO dto) {
    ...
    @GetMapping("/deleted")
    public ResponseEntity<List<InstitutionDTO>> listDeleted() {
